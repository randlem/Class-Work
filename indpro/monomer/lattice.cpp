...
void Lattice::upnbhd(site mysite)
{
	...
	//cout<<"getcte="<<ctr<<endl;
	 checksite(mysite);
	  for(i=0;i<ctr;i++)
	  {
	     x=bondpt[i].x;
	     y=bondpt[i].y;
	     checksite(location[x][y]);//bond=checkupdatebonds(location[bondpt[i].x][bondpt[i].y]);
	  }
...
//boundary event
	  ...
	  /*
	  bdyevent[bdycount].oldsite=location[locx][locy];
	  bdyevent[bdycount].newsite=location[locx][locy];
	  bdyevent[bdycount].t=time;
	  bdycount++;
	  */
...
/*
bond=checkupdatebonds(location[locx][locy]);
//if monomer add to monomer list
if(bond==false)
{
    if(location[locx][locy].index==-1)
    {
       location[locx][locy].index=mcount;
       monomerloc[mcount]=location[locx][locy].pos;
       mcount++;
    }
...
//cout<<"****Diffuson*************************"<<endl;
  ...
  if(mcount>0)
  {
	...
	cout<<"locx"<<oldloc.x<<"locy"<<oldloc.y<<endl;
	cout<<"nlocx"<<newloc.x<<"nlocy"<<newloc.y<<endl;
	//cout<<"bdyevent"<<bdycount<<endl;
