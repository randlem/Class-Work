...
typedef struct __align__(16) {
	...
	int theta;
             ...
             cuda_dim        Solver_dim        = { 256, 256 }; // 8192 votes
...
__host__ void setup_Solver_accum(uint width, uint height) {
	...
	// copy the width and height to constant memory
...
__device__ void accumulator(ellipse_t* ellipse, uint* d_center_accum, uint* d_axes_accum, uint* d_theta_accum) {
	uint *center_ptr, *axes_ptr, *theta_ptr, offset;

	center_ptr = axes_ptr = theta_ptr = NULL;

	offset = ellipse->y * d_width + ellipse->x;
	if (offset < d_width * d_height
			&& ellipse->x >= 0 && ellipse->x < d_width
			&& ellipse->y >= 0 && ellipse->y < d_height)
		center_ptr = &d_center_accum[offset];

	offset = ellipse->b * d_width + ellipse->a;
	if (offset < d_width * d_height
			&& ellipse->a >= 0 && ellipse->a < d_width
			&& ellipse->b >= 0 && ellipse->b < d_height)
	...
	if (center_ptr !=  NULL && axes_ptr != NULL && theta_ptr != NULL) {
		//printf("%d %d: %x %x %x\t%dx%d %dx%d %d\n", blockIdx.x, threadIdx.x, center_ptr, axes_ptr, theta_ptr,ellipse->x,ellipse->y,ellipse->a,ellipse->b,ellipse->theta);
...
__global__ void Solver_kernel(
	...
	edge_pixel_t* pxl;
	extern __shared__ ellipse_t ellipses[];
	ellipse_t ellipse;
	uint *center_ptr, *axes_ptr, *theta_ptr, offset;

	// find the random number index to start at
	...
	for(; cnt > 0; cnt--) {
		...
		if (J > 0.0 || fabs(J) <= FP_PRE) {
			...
			ellipses[threadIdx.x].x     = ellipse.x;
			ellipses[threadIdx.x].y     = ellipse.y;
			ellipses[threadIdx.x].a     = ellipse.a;
			ellipses[threadIdx.x].b     = ellipse.b;
			ellipses[threadIdx.x].theta = ellipse.theta;

			//printf("%d %d: %dx%d %dx%d %d\n", blockIdx.x, threadIdx.x, ellipse->x, ellipse->y, ellipse->a, ellipse->b, ellipse->theta);
		}
	}
	// make sure everybody is done before we go on
	__syncthreads();

	if (threadIdx.x == 0) {
		for(i=0; i < blockDim.x; i++) {
			//printf("%d %d: %dx%d %dx%d %d\n", blockIdx.x, i, ellipses[i].x, ellipses[i].y, ellipses[i].a, ellipses[i].b, ellipses[i].theta);
//			accumulator(&ellipses[i],d_center_accum,d_axes_accum,d_theta_accum);

			center_ptr = axes_ptr = theta_ptr = NULL;

			offset = ellipses[i].y * d_width + ellipses[i].x;
			if (offset < d_width * d_height
					&& ellipses[i].x >= 0 && ellipses[i].x < d_width
					&& ellipses[i].y >= 0 && ellipses[i].y < d_height)
				center_ptr = &d_center_accum[offset];

			offset = ellipses[i].b * d_width + ellipses[i].a;
			if (offset < d_width * d_height
					&& ellipses[i].a >= 0 && ellipses[i].a < d_width
					&& ellipses[i].b >= 0 && ellipses[i].b < d_height)
				axes_ptr   = &d_axes_accum[offset];

			if (ellipses[i].theta >= 0 && ellipses[i].theta < THETA_ACCUM_CNT)
				theta_ptr = &d_theta_accum[ellipses[i].theta];
