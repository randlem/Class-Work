...
void image_create(img_t* img, int width, int height) {
	img->width      = width;
	img->height     = height;
	...
	for (int y=0; y < img->height; y++) {
		...
		memset(img->pixels[y],0,sizeof(color_t) * img->width);
	}
}

void image_copy(img_t* src, img_t* dst) {
	dst->width      = src->width;
	dst->height     = src->height;
	dst->color_type = src->color_type;
	dst->bit_depth  = src->bit_depth;

	dst->pixels     = new color_t*[dst->height];
	for (int y=0; y < dst->height; y++) {
		dst->pixels[y] = new color_t[dst->width];
		memcpy(dst->pixels[y],src->pixels[y],sizeof(color_t)*dst->width);
	}
}

bool image_read(const char *file_name, img_t* img) {
	uchar_t header[8];
	FILE *fp;
	png_structp png;
	png_infop info;
	...
	debug("Loading PNG %s %dx%d (%dbbp)",file_name,img->width,img->height,img->bit_depth);
	...
	memset(row_pointers,0,sizeof(png_bytep) * img->height);
	img->pixels  = new color_t*[img->height];
	memset(img->pixels,0,sizeof(color_t) * img->height);
	for (y=0; y < img->height; y++) {
		row_pointers[y] = new png_byte[info->rowbytes];
		memset(row_pointers[y],0,sizeof(png_byte) * info->rowbytes);
		img->pixels[y]  = new color_t[img->width];
		memset(img->pixels[y],0,sizeof(color_t) * img->width);
	}
...
bool image_write(const char *file_name, img_t* img) {
	FILE *fp;
	png_structp png;
	png_infop info;
//	png_infop end_info;
	...
	png_set_IHDR(png, info, img->width, img->height, img->bit_depth,
		img->color_type, PNG_INTERLACE_NONE, PNG_COMPRESSION_TYPE_BASE,
