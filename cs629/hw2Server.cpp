...
void *serverWorker(void *c_id) {
	...
	cout << client_id << ": is running." << endl;

	// recieve who the messages are going to
	memset(buffer,0,MAX_RECV_SIZE*sizeof(char));
	...
	cout << client_id << ": sending messages to " << send_to << endl;
	...
	send_msg += '\n';
	fclose(in_pipe);
	cout << client_id << ": " << send_msg;

	// send message to client
	ret = send(this_thread->socket,send_msg.c_str(),send_msg.length(),0);
	if (ret == -1) {
		perror("send");
		pthread_exit(NULL);
	}
	cout << client_id << ": finished sending message to client." << endl;
	...
	cout << client_id << ": recieved ack from client. Exiting" << endl;

	// clean up pipe file descriptors & socket
	close(this_thread->socket);
	delete this_thread;
	active_servers[client_id] = NULL;

	cout << client_id << ": is done." << endl;
	pthread_exit(NULL);
}
