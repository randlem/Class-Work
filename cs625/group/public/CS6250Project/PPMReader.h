...
bool ReadPPMB(Image& image, char* fileName)
{
    ...
    // This section of code flips the image upside-down.
    // Since the PPM format specify (0,0) as the pixel on the top left,
    // while OpenGL specify (0,0) as the pixel on the bottom left, the 
    // image need to be inverted to be displayed correctly.
    
    // Flip code begins
    /*
    int totalPixels = w * h;    
    unsigned char* bufTemp = new unsigned char[3 * totalPixels];
    //for (int i = 0; i < totalPixels; i++)
    for (int y = 0; y < h; y++)
        for (int x = 0; x < w; x++)
        {
            unsigned char* rgb = image.GetPixelAt(x, y);
            bufTemp[3 * (x + (h - 1 - y) * w)] = rgb[0];
            bufTemp[3 * (x + (h - 1 - y) * w) + 1] = rgb[1];
            bufTemp[3 * (x + (h - 1 - y) * w) + 2] = rgb[2];
            //bufTemp[3*totalPixels - 3*i - 3] = image.buf[3*i];
            //bufTemp[3*totalPixels - 3*i - 2] = image.buf[3*i+1];
            //bufTemp[3*totalPixels - 3*i - 1] = image.buf[3*i+2];
        }

    memcpy(image.buf, bufTemp, 3 * totalPixels);
...
bool ReadPPMA(Image& image, char* fileName)
{
    ...
    // Flip code begins
    /*
    unsigned char* bufTemp = new unsigned char[3 * w * h];
