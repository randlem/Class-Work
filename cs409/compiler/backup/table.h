...
class TableEntry {
public:
	TableEntry() { link = NULL; }
	virtual ~TableEntry() { }           // virtual destructor
	virtual string getKey() const = 0;  // pure virtual function
	TableEntry* getLink() { return(link); }   // return the next link
	void setLink(TableEntry* e) { link = e; } // set the next link
protected:
	TableEntry* link;                   // used to do chaining
};
...
private:
   ...
   // helper fncts
   int hashKey(const string &) const;
