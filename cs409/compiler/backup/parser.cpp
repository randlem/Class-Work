...
void Parser::BR() {
	...
	// if this is a proc structure delete the LOCAL symbol table
...
void Parser::EB() {
	...
	switch(structure->stType) {
		...
		case STelse: {
			// do nothing...just dump the current struct off the stack
		} break;
		default: {
...
void Parser::xx() {
	...
	setError("Syntax Error! Offending code: " + currentOp->getKey() + " " + operandPtr->getKey() + " " + nextOp->getKey());
